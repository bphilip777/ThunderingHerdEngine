...
pub fn init(allo: Allocator, enable_validation_layers: bool) !Vulkan {
    const instance = try createInstance(allo, enable_validation_layers);
    ...
    // if (is_debug_mode) std.debug.print("Command Buffers Complete\n", .{});
    //
...
fn deinitCommandPool(self: *Vulkan, allo: Allocator) void {
    ...
    allo.free(self.command_buffers);
}

fn deinitSync(self: *Vulkan, allo: Allocator) void {
    for (self.image_available_semaphores, self.render_finished_semaphores, self.in_flight_fences) |ias, rfs, iff| {
        vk.destroySemaphore(self.device, ias, null);
        vk.destroySemaphore(self.device, rfs, null);
        vk.destroyFence(self.device, iff, null);
    }
    allo.free(self.in_flight_fences);
    allo.free(self.image_available_semaphores);
...
fn checkValidationLayerSupport() bool {
    var n_layers: u32 = 0;
    isSuccess(vk.enumerateInstanceLayerProperties(&n_layers, null)) catch |err| {
        std.debug.print("Error: {s}\n", .{@errorName(err)});
        return false;
    };

    var available_layers: [64]vk.LayerProperties = undefined;
    isSuccess(vk.enumerateInstanceLayerProperties(&n_layers, &available_layers)) catch |err| {
        std.debug.print("Error: {s}\n", .{@errorName(err)});
        return false;
    };
    ...
    for (validation_layers) |layer_name| {
        ...
        for (available_layers) |available_layer| {
            const len = std.mem.indexOfScalar(u8, &available_layer.layer_name, 0) orelse available_layer.layer_name.len;
...
fn printAvailableValidationLayers() void {
    var n_layers: u32 = 0;
    isSuccess(vk.enumerateInstanceLayerProperties(&n_layers, null)) catch |err| {
        std.debug.print("Error: {s}\n", .{@errorName(err)});
    };
    ...
    for (available_layers[0..n_layers]) |layer| {
        const len = std.mem.indexOfScalar(u8, &layer.layer_name, 0) orelse layer.layer_name.len;
        const name = layer.layer_name[0..len];
        std.debug.print("\t{s}\n", .{name[0..len]});
    }
}

// pub fn createSurface(window: *sdl.SDL_Window, instance: *const vk.Instance) !vk.SurfaceKHR {
...
// }
//
// fn recreateSwapchain(self: *Vulkan) !void {
//     self.resize = false;
...
//         if (!sdl.SDL_GetWindowSize(self.window, &width, &height)) return error.GettingWindowSizeFailed;
...
// }
//
// fn cleanupSwapchain(self: *Vulkan) void {
//     { // Depth Resources
